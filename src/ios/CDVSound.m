...
- (CDVAudioFile*)audioFileForResource:(NSString*)resourcePath withId:(NSString*)mediaId doValidation:(BOOL)bValidate forRecording:(BOOL)bRecord
{
    ...
    if (bError) {
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:errcode message:errMsg]];
...
- (void)create:(CDVInvokedUrlCommand*)command
{
    ...
    if (audioFile == nil) {
        ...
        NSString* jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_ABORTED message:errorMessage]];
        [self.commandDelegate evalJs:jsString];
    } else {
        ...
        if ((audioFile != nil) && (audioFile.resourceURL != nil)) {
            ...
            if (!bError) {
                ...
                if (!bError) {
                    ...
                    jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%.3f);\n%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_DURATION, position, @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
                    [self.commandDelegate evalJs:jsString];
                }
            }
            if (bError) {
                ...
                // jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_ERROR,  MEDIA_ERR_NONE_SUPPORTED];
                jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_NONE_SUPPORTED message:nil]];
                [self.commandDelegate evalJs:jsString];
            }
        }
...
- (void)stopPlayingAudio:(CDVInvokedUrlCommand*)command
{
    ...
    if ((audioFile != nil) && (audioFile.player != nil)) {
        ...
        [audioFile.player stop];
        audioFile.player.currentTime = 0;
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_STATE, MEDIA_STOPPED];
...
- (void)pausePlayingAudio:(CDVInvokedUrlCommand*)command
{
    ...
    if ((audioFile != nil) && (audioFile.player != nil)) {
        ...
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_STATE, MEDIA_PAUSED];
    }
    // ignore if no media playing
...
- (void)seekToAudio:(CDVInvokedUrlCommand*)command
{
    ...
    if ((audioFile != nil) && (audioFile.player != nil)) {
        ...
        if (posInSeconds >= audioFile.player.duration) {
            ...
            [audioFile.player stop];
            audioFile.player.currentTime = 0;
            jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%.3f);\n%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_POSITION, 0.0, @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_STATE, MEDIA_STOPPED];
            // NSLog(@"seekToEndJsString=%@",jsString);
        } else {
            audioFile.player.currentTime = posInSeconds;
            jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%f);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_POSITION, posInSeconds];
...
#pragma unused(mediaId)
    ...
    NSString* jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%.3f);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_POSITION, position];
    [self.commandDelegate evalJs:jsString];
            ...
            void (^startRecording)(void) = ^{
                ...
                if ([self hasAudioSession]) {
                    ...
                    if (![self.avSession setActive:YES error:&error]) {
                        ...
                        // jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_ERROR, MEDIA_ERR_ABORTED];
                        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_ABORTED message:errorMsg]];
                        [self.commandDelegate evalJs:jsString];
                        return;
                    }
                }
                ...
                if (error == nil) {
                    ...
                    if (recordingSuccess) {
                        ...
                        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
                        [self.commandDelegate evalJs:jsString];
                    }
                }
    ...
    #pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                [self.avSession performSelector:rrpSel withObject:^(BOOL granted){
                    if (granted) {
                        ...
                        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_ABORTED message:msg]];
                        [self.commandDelegate evalJs:jsString];
                    }
                }];
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder*)recorder successfully:(BOOL)flag
{
    ...
    if (flag) {
        ...
        // jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_ERROR, MEDIA_ERR_DECODE];
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-with-compression-fork.Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_DECODE message:nil]];
    }
    if (self.avSession) {
        [self.avSession setActive:NO error:nil];
    }
